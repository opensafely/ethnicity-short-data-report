...
## import data
eth_ons_input<-read_csv(here::here("data","ethnicity_ons_2021.csv.gz"))
...
ethnicity <-
  # read_csv(here::here("output","sus","simplified_output","5_group","tables","simple_patient_counts_categories_registered.csv"),col_types =(cols())) %>%
...
write_csv(ethnicity1,here::here("output", "from_jobserver","release_2022_11_18","made_locally","ethnic_group_registered_2021.csv")) 

##### 16 group
ethnicity_16 <-
  read_csv(here::here("output","sus","simplified_output","16_group","tables","simple_patient_counts_categories_registered.csv"),col_types =(cols())) %>%
  filter(group=="region" | group=="all") %>%
  select(-population)

ethnicity2 <- ethnicity_16 %>%
  rename_with(~sub("ethnicity_","",.),contains("ethnicity_")) %>%
  ...
  left_join(population,by=c("subgroup","codelist")) %>%
  mutate(percentage = round(N / Total *100,1),
         across('ethnicity', str_replace, '_ethnicity_new_16_filled', '')
  ) %>%
  summarise(
    region=case_when(subgroup=="with records"~"England",
                    TRUE~subgroup),
